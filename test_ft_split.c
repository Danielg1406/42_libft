...
void	test_ft_split(void)
{
	...
	if (strcmp(result1[0], "Hello") == 0 && strcmp(result1[1], "World") == 0 && result1[2] == NULL)
		printf("PASS: ft_split(\"%s\", ' ') = [\"%s\", \"%s\"]\n", str1, result1[0], result1[1]);
	else
		printf("FAIL: ft_split(\"%s\", ' ') = [\"%s\", \"%s\"]," "expected [\"Hello\", \"World\"]\n", str1, result1[0], result1[1]);
	free(result1);
	str2 = "This,is,a,test";
	...
	if (strcmp(result2[0], "This") == 0 && strcmp(result2[1], "is") == 0
		...
		&& result2[4] == NULL)
		printf("PASS: ft_split(\"%s\", ',') = [\"%s\", \"%s\", \"%s\"," "\"%s\"]\n", str2, result2[0], result2[1], result2[2], result2[3]);
	else
		printf("FAIL: ft_split(\"%s\", ',') = [\"%s\", \"%s\", \"%s\", \"%s\"]," "expected [\"This\", \"is\", \"a\", \"test\"]\n", str2, result2[0], result2[1], result2[2], result2[3]);
	free(result2);
	...
	if (strcmp(result3[0], "One") == 0 && strcmp(result3[1], "Two") == 0
		...
		printf("PASS: ft_split(\"%s\", ',') = [\"%s\", \"%s\", \"%s\", \"%s\"," "\"%s\"]\n", str3, result3[0], result3[1], result3[2], result3[3], result3[4]);
	else
		printf("FAIL: ft_split(\"%s\", ',') = [\"%s\", \"%s\", \"%s\", \"%s\"," "\"%s\"], expected [\"One\", \"Two\", \"Three\", \"Four\"," "\"Five\"]\n", str3, result3[0], result3[1], result3[2], result3[3], result3[4]);
	free(result3);
	str4 = "hello!";
